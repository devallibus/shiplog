...
5. **Link and store.** PR body includes `Closes #<issue>` when the PR fully resolves the issue. For partial delivery, use `Addresses #<issue> (completes T1, T2, ...)` — see the partial-delivery template below and `references/closure-and-review.md` §1. Store key learning in knowledge graph.
...
**When to use:** Any time a PR merges work but the issue should stay open. After merge, post a `[shiplog/milestone]` comment on the issue and a `[shiplog/blocker]` comment for externally-blocked tasks.

...
| `shiplog/ready` | The issue is fully scoped and ready to implement (no blockers, no open tier-1 decisions) | `2DA44E` | Ready to implement |
| `shiplog/in-progress` | Work has started — a branch exists and an agent is actively working | `FBCA04` | Implementation in progress |
| `shiplog/needs-review` | A PR exists and is awaiting cross-model review before merge | `D93F0B` | Awaiting review |

**Lifecycle label rules:** `shiplog/ready`, `shiplog/in-progress`, and `shiplog/needs-review` are mutually exclusive. Only one may be active on an issue at a time. Apply the label that matches the current state and remove any previously active lifecycle label.
...
| Issue envelope `readiness: ready` and no lifecycle label present | add `shiplog/ready` |
| Branch created for issue (Phase 2) | replace any lifecycle label with `shiplog/in-progress` |
| PR created for issue (Phase 5) | replace any lifecycle label with `shiplog/needs-review` |
| PR merged and issue closed | remove lifecycle labels |
...
# Lifecycle label transitions (mutually exclusive — remove old before adding new):
...
# On close: remove all lifecycle labels
